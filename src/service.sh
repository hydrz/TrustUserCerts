...
fix_permissions() {
    ...
    # Try both contexts for better compatibility across Android versions
    chcon -R u:object_r:system_file:s0 "$target_dir"/* 2>/dev/null
    chcon -R u:object_r:system_security_cacerts_file:s0 "$target_dir"/* 2>/dev/null
}
    ...
    while true; do
        ...
        for zp in $zygote_pids; do
            ...
            if ! has_mount "$zp"; then
                ...
                # Inject mount into all child processes
